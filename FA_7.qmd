...
# This chunk ensures essential NLTK resources are present
# BEFORE the textual module (C2) is imported, resolving LookupErrors.
try:
    import nltk
    ...
    # If NLTK isn't installed yet, this will be handled in C7
...
The research results presented in this brief are influenced by multiple limitations in data and methodology. The calculation of vacancy rate is based on all houses not just those available for rent due to the inability to obtain vacancy rate data for the rental market. This makes the indicator an approximation as some vacant houses may be occupied by their owners or unavailable for rent. Consequently, the vacancy rate of Airbnb is also an approximation. 

Additionally, data from Inside Airbnb is a snapshot at a specific point in time and may not reflect market dynamics due to the constant increase or deletion of housing information. This could lead to overestimation or underestimation in the statistics of seasonal or temporary housing. Furthermore, the process of web scraping for this dataset may contain errors resulting in an underestimation of the number of housing units during data cleaning. For example, duplicate IDs or entries with more than six empty fields have been deleted. 
