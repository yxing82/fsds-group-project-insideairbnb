---
title: We Love Python's Group Project
subtitle: Data Analysis and Policy Briefing on Airbnb and Professional Landlords in London
format:
  pdf:
...
Full code and data can be found at the attached (<https://github.com/yxing82/fsds-group-project-insideairbnb>).

```{python}
#| echo: false
#| output: false
# --- CRITICAL PRE-IMPORT NLTK DOWNLOADS ---
try:
    import nltk
    # Note: Using nltk.download() is safe here as it checks for existence first.
    nltk.download("averaged_perceptron_tagger")
    
    # Force load of the tagger resource to ensure path is cached
    from nltk.tag import PerceptronTagger
    PerceptronTagger() 
    print("Pre-downloaded all required NLTK resources.")
except ImportError:
    print("NLTK not yet installed. Proceeding to C7.")
except Exception as e:
    print(f"NLTK pre-download failed: {e}. Attempting full download in C8.")
```
...
# ====================================================================
# A. IPython Magics (Must be at the very top of the chunk)
# ====================================================================
%load_ext autoreload
%autoreload 2
...
# ====================================================================
# B. Standard Library and Assumed Base Imports
# ====================================================================
from pathlib import Path  # Standard Library
from functools import wraps  # Standard Library
import urllib.request # Standard Library (Needed for textual download)

# These are highly likely to be base packages. Keep them as straight imports.
import numpy as np
import pandas as pd
...
# ====================================================================
# C. Conditional Installation Blocks (In Dependency Order)
# ====================================================================

# --- C1. Conditional Installation for Unidecode (Dependency for textual) ---
try:
    from unidecode import unidecode
    print("Unidecode is present. Proceeding.")
except ImportError:
    print("Unidecode not found. Attempting installation...")
    ! pip install unidecode
    from unidecode import unidecode
    print("Unidecode successfully installed and imported.")

# --- C7. Consolidated NLTK Installlation and Import ---
try:
    import nltk
    print("NLTK is present. Proceeding.")
except ImportError:
    print("NLTK not found. Attempting installation...")
    ! pip install nltk
    import nltk
    print("NLTK successfully installed and imported.")


# --- C8. CONDITIONAL NLTK RESOURCE DOWNLOADS (Relies on NLTK being installed C7) ---
try:
    # Attempt to import all required resources/components
    from nltk.corpus import wordnet as wn
    from nltk.stem.wordnet import WordNetLemmatizer
    from nltk.corpus import stopwords
    from nltk.tokenize.toktok import ToktokTokenizer  
    from nltk.tag import PerceptronTagger # Include this import
    
    # Test that the resource (e.g., 'stopwords') is available by using it
    stopwords.words('english')
    PerceptronTagger() # Test that the tagger is available
    
    # Initialize objects if the resources are present
    ...
    # If any resource lookup fails, download them
    print("Missing NLTK resources found. Attempting to download them...")

    # We assume NLTK is installed from C7
    ...
    # --- Force load the tagger immediately after downloading ---
    try:
        from nltk.tag import PerceptronTagger
        PerceptronTagger() # Force the loading to complete and cache the path
        print("Averaged Perceptron Tagger successfully loaded.")
    except Exception as e:
        print(f"Warning: Could not force load PerceptronTagger: {e}")
    
    # Re-import and re-initialize objects
    # Note: These re-imports are necessary to register the freshly downloaded resources
    from nltk.corpus import stopwords
    from nltk.corpus import wordnet as wn
    from nltk.stem.wordnet import WordNetLemmatizer
    from nltk.tokenize.toktok import ToktokTokenizer
    ...
    lemmatizer = WordNetLemmatizer()
    tokenizer = ToktokTokenizer()
    print("All NLTK resources downloaded and initialized.")


# --- C2. Custom Module (textual) Conditional Import and Download ---
# NOTE: This relies on Path (B) and unidecode (C1) being available.
try:
    from textual import *
    print("Custom module 'textual' is present.")
except ImportError:
    print("Custom module 'textual' not found. Attempting to download...")
    ...
    if not tpath.exists():
        ...
        print(f"Downloaded 'textual' module to {tpath}")
    else:
        print("'textual' module folder already exists locally.")
        
    from textual import *
    print("Custom module 'textual' successfully downloaded and imported.")


# --- C3. Conditional Installation for Requests ---
try:
    # MUST be 'from requests import get'
    from requests import get 
    print("Requests is present. Proceeding.")
except ImportError:
    print("Requests not found. Attempting installation...")
    ! pip install requests
    # MUST be 'from requests import get' here too
    from requests import get 
    print("Requests successfully installed and imported.")


# --- C4. Conditional Installation for Geopandas (Relies on Numpy/Pandas/Matplotlib from B) ---
try:
    import geopandas as gpd
    print("Geopandas is present. Proceeding.")
except ImportError:
    print("Geopandas not found. Attempting installation...")
    ! pip install geopandas
    import geopandas as gpd
    print("Geopandas successfully installed and imported.")


# --- C5. Conditional Installation for BeautifulSoup (bs4) ---
try:
    from bs4 import BeautifulSoup
    print("BeautifulSoup is present. Proceeding.")
except ImportError:
    print("BeautifulSoup not found. Attempting installation...")
    ! pip install beautifulsoup4
    from bs4 import BeautifulSoup
    print("BeautifulSoup successfully installed and imported.")


# --- C6. Conditional Installation for scikit-learn (sklearn) ---
try:
    # Attempt to import all necessary sklearn components
    from sklearn.feature_extraction.text import TfidfVectorizer
    from sklearn.feature_extraction.text import CountVectorizer
    from sklearn.decomposition import LatentDirichletAllocation
    from sklearn.metrics.pairwise import cosine_similarity 
    print("scikit-learn is present. Proceeding.")
except ImportError:
    print("scikit-learn not found. Attempting installation...")
    # The ! pip install command runs only if the import fails
    ! pip install scikit-learn
    
    # Re-attempt the import after installation
    from sklearn.feature_extraction.text import TfidfVectorizer
    from sklearn.feature_extraction.text import CountVectorizer
    from sklearn.decomposition import LatentDirichletAllocation
    from sklearn.metrics.pairwise import cosine_similarity 
    print("scikit-learn successfully installed and imported.")



# ====================================================================
# D. Final Utility Imports and Global Variable Initialization (Relies on C7/C8)
# ====================================================================
...
We will explore whether Airbnb in London is out of control from three aspects:

### 1.1 How do Airbnb listings distribute in London?

We begin the analysis by investigating the density of Airbnb listings in London using a map of the distribution of each room type.
...
try:
    ...
    if boros_map.crs is None or boros_map.crs.to_epsg() != 4326:
        boros_map = boros_map.set_crs(epsg=4326, allow_override=True)
    
    # Re-project to the British National Grid (EPSG:27700). 
    # This coordinate system uses meters and corrects the shape distortion for the UK.
    boros_map = boros_map.to_crs(epsg=27700)
    print("Map re-projected to British National Grid (EPSG:27700) to fix aspect ratio.")
    # --------------------------------------------------------
...
# --- 1. AGGRESSIVE CLEANING (Fixes Plot ValueError) ---
boros_dwell_vac = boros_dwell_vac[boros_dwell_vac.geometry.is_valid].copy()
boros_dwell_vac = boros_dwell_vac[~boros_dwell_vac.geometry.isna()].copy()
boros_dwell_vac = boros_dwell_vac.reset_index(drop=True)


# --- 2. DEFINE THRESHOLDS (Fixes NameError) ---
# MUST be defined after cleaning
...
# The plot will now use the projected coordinates
boros_dwell_vac.plot(color=boros_dwell_vac['color'], edgecolor='black', linewidth=0.8, ax=ax)

# --- MANUAL ASPECT RATIO SETTING (Bypasses error-prone calculation) ---
# This line works correctly now that the underlying geometry is projected
ax.set_aspect('equal')

# Annotate
for idx, row in boros_dwell_vac.iterrows():
    # Annotations use the projected coordinates
...
The research results presented in this brief are influenced by multiple limitations in data and methodology. The calculation of the vacancy rate is based on all houses not just those available for rent due to the inability to obtain vacancy rate data for the rental market. This makes the indicator an approximation as some vacant houses may be occupied by their owners or unavailable for rent. Consequently, the vacancy rate of Airbnb is also an approximation.

Additionally, data from Inside Airbnb is a snapshot at a specific point in time and may not reflect market dynamics due to the constant addition or removal of listings. This could lead to overestimation or underestimation in the statistics of seasonal or temporary housing. Furthermore, the process of web scraping for this dataset may contain errors resulting in an underestimation of the number of housing units during data cleaning. For example, duplicate IDs or entries with more than six empty fields have been deleted.

Finally, the housing stock data is from 2021 which may lead to minor deviations and other key factors affecting housing supply or Airbnb distribution (such as local regulations, rental demand, or real estate prices) are not included in this analysis.
...
The opposition’s  proposal helps the Mayor demonstrate responsiveness and leadership on the highly salient public concern of housing. Secondly, because the proposal targets professional landlords and areas where short-term rental pressures are most concentrated, it can also be presented as a proportionate and targeted intervention which reduces the political cost of decisive action. Additionally, the proposal helps shift the conversation away from a personal scandal involving the Mayor’s advisor and towards a system-level regulatory challenge. Finally, mandatory registration creates a formal need for platform-held data, which makes it easier to push for structured data-sharing arrangements. However, implementing this proposal will present several potential risks. Firstly, as the Mayor did not introduce the policy initially, any implementation difficulties or a gap between promises and outcomes will be interpreted as evidence of weak leadership. Additionally, registration and licensing regimes are administratively complex and resource-intensive, which is difficult to deliver at scale. Experience in Scotland shows that delays in processing and limited enforcement capacity can turn the policy from a sign of control into evidence of administrative weakness. 
